...
public class NetworkingSettings
{
    ...
    public NetworkingSettings(bool loadSettings = false)
    {
        if (loadSettings)
        {
            ...
            CurrentUser.JwtToken = ProtectedSecretStore.Current.Get("networking.jwt") ?? string.Empty;
    ...
    public void Save()
    {
        if (!string.IsNullOrEmpty(CurrentUser.JwtToken))
            ProtectedSecretStore.Current.Set("networking.jwt", CurrentUser.JwtToken);
        else
            ProtectedSecretStore.Current.Remove("networking.jwt");

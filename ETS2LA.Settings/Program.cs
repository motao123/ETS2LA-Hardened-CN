...
namespace ETS2LA.Settings
{
    public class SettingsHandler : IDisposable
    {
        ...
        public static string ConfigurationDirectory =>
            Path.GetFullPath(
                Environment.GetEnvironmentVariable("ETS2LA_CONFIG_DIR") is { Length: > 0 } configured
                    ? configured
                    : Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "ETS2LA-Hardened-CN"));
        ...
        {
            _baseDir = ConfigurationDirectory;
        ...
        private string ResolveJsonPath(string fileName)
        {
            if (string.IsNullOrWhiteSpace(fileName) ||
                !fileName.EndsWith(".json", StringComparison.OrdinalIgnoreCase) ||
                Path.IsPathFullyQualified(fileName))
                throw new ArgumentException("Settings file name must be a relative .json path.", nameof(fileName));

            var root = Path.TrimEndingDirectorySeparator(Path.GetFullPath(_baseDir));
            ...
            if (!candidate.StartsWith(prefix, OperatingSystem.IsWindows() ? StringComparison.OrdinalIgnoreCase : StringComparison.Ordinal))
                throw new ArgumentException("Settings path must remain inside the configuration directory.", nameof(fileName));
            return candidate;
        }

        public bool Save<T>(string fileName, T data)
        {
            string target = ResolveJsonPath(fileName);
            ...
            try
            {
                ...
                if (!saved)
                    throw new IOException($"Could not atomically save settings file '{fileName}' after several attempts.");
        ...
        {
            string path = ResolveJsonPath(fileName);
            if (!File.Exists(path))
            {
